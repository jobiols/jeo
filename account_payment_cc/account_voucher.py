        ...
        for voucher in self.browse(cr, uid, ids):
            if voucher.journal_id.support_creditcard_transactions:
                ...
                move_id = move_pool.create(cr, uid,
                                           self.account_move_get(cr, uid, voucher.id,
                                                                 context=context),
                                           context=context)
                ...
                if alines:
                    for line in voucher.line_ids:
                        ...
                        if not line.amount:
                            continue
                        amount = self._convert_amount(cr, uid, line.amount, voucher.id,
                                                      context=ctx)
                    ...
                    amount = self._convert_amount(cr, uid, (credit + debit), voucher.id,
                                                  context=ctx)
                    line_debit = line_credit = 0.0
                    if voucher.type in ('purchase', 'payment'):
                        line_credit = amount
    ...
    def cancel_voucher(self, cr, uid, ids, context=None):
        ...
        for voucher in self.browse(cr, uid, ids, context=context):
            ...
            if voucher_number and voucher.journal_id.support_creditcard_transactions:
                cc_move = move_pool.search(cr, uid, [("name", "=", voucher_number)],
                                           context=context)
