...
class account_invoice(osv.osv):
    ...
    def invoice_pay_customer(self, cr, uid, ids, context=None):
        ...
        res = super(account_invoice, self).invoice_pay_customer(cr, uid, ids,
                                                                context=context)
        rec = self.browse(cr, uid, ids[0], context=context)
        move_lines = move_line_obj.search(cr, uid, [('invoice', '=', rec.id)])
        voucher_id = None
        if move_lines:
            voucher_lines = voucher_line_obj.search(cr, uid,
                                                    [('move_line_id', 'in', move_lines)])
            if voucher_lines:
                for voucher_line in voucher_line_obj.browse(cr, uid, voucher_lines,
                                                            context):
        ...
        res.update({'view_id':
                        self.pool.get('ir.model.data').get_object_reference(cr, uid,
                                                                            'account_voucher',
                                                                            view)[1],
                    'target': 'current'})
        return res
