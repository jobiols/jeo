...
class account_voucher(osv.osv):
    def setParameter(self, parameters={}, key=None, value=None):
        ...
        if key != None and value != None and str(key).strip() != '' and str(
                value).strip() != '':
            parameters[key] = str(value).strip()
        else:
            raise AuthnetAIMError(
                'Incorrect parameters passed to setParameter(): {0}:{1}'.format(key,
                                                                                value))
        return parameters
        ...
        if isinstance(vals, list):
            for val in vals:
                if val.get('cc_number', False):
                    dec_data = rsa_encrypt.decrypt(val.get('cc_number', False),
                                                   val.get('key', ''))
                    val['cc_number'] = dec_data
                    if context.get('cc_no', '') != 'no_mask':
                        i = len(val['cc_number']) - 4
                        val['cc_number'] = 'X' * i + val['cc_number'][
                                                     -4:len(val['cc_number'])]
        else:
            if vals.get('cc_number', False):
                dec_data = rsa_encrypt.decrypt(vals.get('cc_number', False),
                                               vals.get('key', ''))
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        default.update(vals)
        return super(account_voucher, self).copy(cr, uid, id, default=default,
                                                 context=context)
    ...
    def onchange_amount(self, cr, uid, ids, amount, rate, partner_id, journal_id,
        ...
        res = super(account_voucher, self).onchange_amount(cr, uid, ids, amount, rate,
    ...
    _columns = {
        ...
        'trans_history_ids': fields.one2many('transaction.details', 'voucher_id',
                                             'Transaction History')
    }
    _defaults = {
    ...
    def onchange_cc_details(self, cr, uid, ids, cc_details, context={}):
        ...
        if cc_details:
            ...
            bank = self.pool.get('res.partner.bank').browse(cr, uid, cc_details,
                                                            context=context)
    ...
    def onchange_partner_id(self, cr, uid, ids, partner_id, journal_id, price,
            currency_id, ttype, date, context={}, sale_id=False):
        if not context:
            context = {}
        if not journal_id:
            return {}
        ...
        cardholder_details = self._get_cardholder_details(cr, uid, ids, partner_id,
                                                          context=context)
        if journal_id and cardholder_details:
            journal_read = self.pool.get('account.journal').read(cr, uid, journal_id,
                                                                 ['cc_allow_processing',
                                                                  'cc_allow_refunds'])
            ...
            res['value']['cc_reseller'] = cardholder_details[
                                              'title'] == 'Reseller' and True or False
        ...
        if sale_id and res['value'].get('line_cr_ids'):
            ...
            for line in res['value']['line_cr_ids']:
                for invoice in sale.invoice_ids:
                    if not (line.get('invoice_id') and line.get('invoice_id') and line[
    ...
    def recompute_voucher_lines(self, cr, uid, ids, partner_id, journal_id, price,
        ...
        if context.get('sale_id'):
            INV_IDS = [x.id for x in
                       self.pool.get('sale.order').browse(cr, uid, context['sale_id'],
        ...
        for line in account_move_lines:
            ...
            if line.currency_id and currency_id == line.currency_id.id:
                ...
                amount_original = currency_pool.compute(cr, uid, company_currency,
                                                        currency_id,
            ...
            rs = {
                ...
                'amount': (move_line_found == line.id) and min(abs(price),
                                                               amount_unreconciled) or 0.0,
    ...
    def _historise(self, cr, uid, id, trans_type='Pre-Auth', trans_id=False, status='',
            amount=0.0, context=None):
        if context is None:
            context = {}
...
class res_company(osv.osv):
    ...
    _columns = {
        ...
        'cc_journal_id': fields.many2one('account.journal', 'Payment Method',
                                         help="The default payment method on payment voucher open using the Pay button from sale order."),
    }
