...
from .unit.binder import TiendaNubeBinder
...
def tienda_nube_product_stock_updated(
...
def delay_unlink(session, model_name, record_id, priority=20):
    ...
    binder = env.get_connector_unit(TiendaNubeBinder)
...
def delay_unlink_all_bindings(session, model_name, record_id, priority=20):
    ...
    model = session.env[model_name]
    record = model.browse(record_id)
    for bind_record in record.tienda_nube_bind_ids:
        tienda_nube_model_name = bind_record._name
        env = get_environment(
                session, tienda_nube_model_name, bind_record.backend_id.id)
        binder = env.get_connector_unit(TiendaNubeBinder)
        ext_id = binder.to_backend(bind_record.id)
        if ext_id:
            export_delete_record.delay(
                    session, tienda_nube_model_name,
...
def delay_export_tracking_number(session, model_name, record_id):
    ...
    for binding in picking.sale_id.tienda_nube_bind_ids:
