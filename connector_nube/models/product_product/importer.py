...
from ...unit.backend_adapter import GenericAdapter, TiendaNubeCRUDAdapter
...
@tienda_nube
class ProductCombinationRecordImport(TiendaNubeImporter):
    ...
    def _import_dependencies(self):
        ...
        backend_adapter = self.unit_for(
                BackendAdapter, 'tienda_nube.product.combination.option.value')
    ...
    def set_variant_images(self, combinations):
        backend_adapter = self.unit_for(
                TiendaNubeCRUDAdapter, 'tienda_nube.product.combination')
        for combination in combinations:
            try:
                ...
                binder = self.binder_for('tienda_nube.product.image')
    ...
    def import_supplierinfo(self, erp_id):
        ps_id = self._get_tienda_nube_data()['id']
        filters = {
        ...
        ps_supplierinfos = self.env['tienda_nube.product.supplierinfo']. \
...
class ProductCombinationMapper(ImportMapper):
    ...
    def main_template(self, record):
        ...
        return self.env['tienda_nube.product.template'].browse(template_id)
    ...
    def _template_code_exists(self, code):
        ...
        combination_binder = self.binder_for('tienda_nube.product.combination')
    ...
    def ean13(self, record):
        if record['ean13'] in ['', '0']:
            backend_adapter = self.unit_for(
                    GenericAdapter, 'tienda_nube.product.template')
    ...
    def _get_tax_ids(self, record):
        ...
        tax_group = self.binder_for('tienda_nube.account.tax.group').to_odoo(
    ...
    def specific_price(self, record):
        product = self.binder_for(
                'tienda_nube.product.combination').to_odoo(
...
@tienda_nube
class ProductCombinationOptionRecordImport(TiendaNubeImporter):
    ...
    def run(self, ext_id):
        ...
        self.tienda_nube_id = ext_id
        self.tienda_nube_record = self._get_tienda_nube_data()
        name = self.mapper.name(self.tienda_nube_record)['name']
        attribute_ids = self.env['product.attribute'].search([
            ('name', '=', name),
        ])
        if len(attribute_ids) == 0:
            # if we don't find it, we create a tienda_nube_product_combination
            super(ProductCombinationOptionRecordImport, self).run(ext_id)
        else:
            # else, we create only a tienda_nube.product.combination.option
            data = {
            ...
            self.binder.bind(self.tienda_nube_id, erp_id.id)
        self._import_values()
...
class ProductCombinationOptionValueAdapter(GenericAdapter):
    _model_name = 'tienda_nube.product.combination.option.value'
    _tienda_nube_model = 'product_option_values'
...
class ProductCombinationOptionValueRecordImport(TranslatableRecordImporter):
    ...
    _translatable_fields = {
        'tienda_nube.product.combination.option.value': ['name'],
    }
...
class ProductCombinationOptionValueMapper(ImportMapper):
    ...
    def attribute_id(self, record):
        binder = self.binder_for('tienda_nube.product.combination.option')
...
class ProductProductBatchImporter(DelayedBatchImporter):
    _model_name = 'tienda_nube.product.product'
