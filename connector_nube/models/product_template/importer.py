...
class TemplateMapper(ImportMapper):
    ...
    def get_sale_price(self, record, tax):
        price_adapter = self.unit_for(
                GenericAdapter, 'tienda_nube.product.combination')
    ...
    def categ_id(self, record):
        ...
        category = self.binder_for(
                'tienda_nube.product.category').to_odoo(
    ...
    def type(self, record):
        ...
        # The same if the product is a virtual one in tienda_nube.
    ...
    def tags_to_text(self, record):
        ...
        tag_adapter = self.unit_for(GenericAdapter, '_tienda_nube_product_tag')
...
class ManufacturerProductImportMapper(ImportMapper):
    # For extend in connector_tienda_nube_manufacturer module, by this way we
...
@tienda_nube
class ProductInventoryImport(TiendaNubeImporter):
    _model_name = ['_import_stock_available']
...
class TemplateRecordImport(TranslatableRecordImporter):
    ...
    def import_combinations(self):
        tienda_nube_record = self._get_tienda_nube_data()
        associations = tienda_nube_record.get('associations', {})
        ...
        if combinations:
            first_exec = combinations.pop(
                    combinations.index({
                        'id': tienda_nube_record[
                            'id_default_combination']['value']}))
            if first_exec:
                import_record(
                        self.session, 'tienda_nube.product.combination',
    ...
    def _import_categories(self):
        ...
        for category in categories:
            self._import_dependency(
                    category['id'], 'tienda_nube.product.category')
