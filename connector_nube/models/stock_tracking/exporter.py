...
@tienda_nube
class TiendaNubeTrackingExporter(Exporter):
    ...
    def run(self, binding_id):
        ...
        tracking_adapter = self.unit_for(
                TiendaNubeCRUDAdapter, '__not_exit_tienda_nube.order_carrier')

        self.binding = self.env[self.model._name].browse(binding_id)
        tracking = self._get_tracking()
        if tracking:
            tienda_nube_order_id = self.binder.to_backend(binding_id)
            filters = {
                'filter[id_order]': tienda_nube_order_id,
            }
...
def export_tracking_number(session, model_name, record_id):
    ...
    tracking_exporter = env.get_connector_unit(TiendaNubeTrackingExporter)
